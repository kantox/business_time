...
One might specify the holidays depending on the currency. That approach
likely fails on `EUR` but here we go:
...
All the methods, calculating working days between two dates, are now ready to
receive an optional currency parameter, to match holidays:

```ruby
deadline = 1.business_day('EUR').after(Date.parse('2015-04-29'))
#⇒ #<Date: 2015-04-30 ...>
deadline = 1.business_day.after(Date.parse('2015-04-30'))
#⇒ #<Date: 2015-05-01 ...>
deadline = 1.business_day('EUR').after(Date.parse('2015-04-30'))
#⇒ #<Date: 2015-05-04 ...>
```

The latter gives _May 4_ because there is a holiday for _May 1_, defined in
`BusinessTime::Config.currency_holidays`.

Unless the currency is passed to those methods, the code behaves as before,
assuming there are no holidays save for standard ‘cross-country’.
